...
export function OrdersProvider({ children }) {
  ...
  const updateItemStatus = (orderId, itemId, status, scannedAt = null) => {
    dispatch({ 
      type: ACTIONS.UPDATE_ITEM_STATUS, 
      payload: { orderId, itemId, status, scannedAt } 
    });
  };

  const scanBarcode = (orderId, itemId, scannedBarcode) => {
    const order = state.orders.find(o => o.id === orderId);
    const item = order?.items.find(i => i.id === itemId);
    
    if (item && item.barcode === scannedBarcode) {
      updateItemStatus(orderId, itemId, ITEM_STATUS.SCANNED, new Date().toISOString());
      return { success: true, message: 'Item scanned successfully!' };
    } else {
