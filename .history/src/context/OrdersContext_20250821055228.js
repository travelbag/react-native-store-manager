...
export function OrdersProvider({ children }) {
  ...
  const initializeNotifications = async () => {
    try {
      ...
      // REMOVE THIS IN PRODUCTION - Demo timer
  ...
  const registerStoreManagerToken = async (pushToken) => {
    try {
      ...
      const response = await fetch('YOUR_API_BASE_URL/store-managers/register-token', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          storeManagerId: 'STORE_MANAGER_ID', // Your store manager ID
          storeId: 'STORE_ID', // Your store ID  
          pushToken: pushToken,
          deviceInfo: {
      ...
      if (response.ok) {
        console.log('Store manager token registered successfully');
      } else {
  ...
  const fetchOrderDetails = async (orderId) => {
    try {
      const response = await fetch(`YOUR_API_BASE_URL/orders/${orderId}`, {
        headers: {
